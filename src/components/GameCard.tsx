      ...
      <div className="rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 border border-transparent group-hover:border-gray-600">
        ...
        <div className="pt-2 sm:pt-3 pb-2 sm:pb-3">
          <div className="flex items-start justify-between gap-1 sm:gap-2">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-[12px] font-semibold text-teal-300 flex-shrink-0">
              {hasRating && <span className="text-teal-300">★</span>}
        ...
        <div className="bg-[#131313] rounded-2xl overflow-hidden shadow-2xl border border-gray-600">
          ...
          <div className="p-4">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-bold text-teal-300 text-[15px] leading-[20px] line-clamp-1 flex-1 min-w-0">
                {game.title}
              </h3>
              <div className="flex items-center gap-1 text-[13px] font-semibold text-teal-300 flex-shrink-0">
                {hasRating && <span className="text-teal-300">★</span>}
                <span>{ratingText}</span>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-1">
              {game.tags.filter(Boolean).map((tag, index) => (
                <span 
                  key={index}
                  className="px-2 py-1 text-[10px] text-teal-300 bg-teal-400/20 rounded-full"
                >
                  {tag}
                </span>
              ))}
